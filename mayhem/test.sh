#!/usr/bin/env bash
#
# parliament/mayhem/test.sh — RUN parliament's own pytest suite and emit a CTRF summary.
# exit 0 iff no test failed.
#
# PATCH-grade behavioral oracle: tests/unit/*.py are known-answer cases asserting that
# analyze_policy_string(<policy>).finding_ids / findings EXACTLY match expected IAM-lint
# results. A no-op / "exit(0)" / behavior-altering patch to parliament cannot pass it.
#
# The suite is run through the `/mayhem/parliament-tests` ELF launcher (which exec()s
...
# executable. The system `python3` under /usr/bin is SPARED, but `/mayhem/parliament-tests`
# is not — so under sabotage the launcher exits before pytest runs, no RUNTESTS line is
# produced, and this script reports a failure. The normal run is unaffected.
#
...
if [ ! -x /mayhem/parliament-tests ]; then
  echo "FATAL: /mayhem/parliament-tests launcher missing — run mayhem/build.sh first" >&2
  emit_ctrf "pytest" 0 1 0
  exit 2
fi

echo "=== running pytest (parliament known-answer suite) via /mayhem/parliament-tests ==="
out="$(/mayhem/parliament-tests 2>&1)"; rc=$?
echo "$out"
